/* title: Services-Multithread impl
   Implements <initmultithread_umgebungservices>, <freemultithread_umgebungservices>.
...
int freemultithread_umgebungservices(umgebung_services_t * svc)
{
   int err = 0 ;
   int err2 ;
...
// TEXTDB:SELECT("   case "row-id":     err2 = freeumgebung_"module"("(if (parameter!="") "&svc->" else "")parameter") ;"\n"               if (err2) err = err2 ;")FROM(C-kern/resource/text.db/initumgebung)WHERE(type=='multi')DESCENDING
   case 2:     err2 = freeumgebung_logwritermt(&svc->ilog) ;
               if (err2) err = err2 ;
   case 1:     err2 = freeumgebung_objectcachemt(&svc->objectcache) ;
               if (err2) err = err2 ;
// TEXTDB:END
   case 0:     break ;
   }
...
int initmultithread_umgebungservices(/*out*/umgebung_services_t * svc)
{
   int err ;
...
// TEXTDB:SELECT(\n"   ONERROR_testerrortimer(&s_error_initres, ABBRUCH) ;"\n"   err = initumgebung_"module"("(if (parameter!="") "&svc->")parameter") ;"\n"   if (err) goto ABBRUCH ;"\n"   ++svc->resource_count ;")FROM(C-kern/resource/text.db/initumgebung)WHERE(type=='multi')

   ONERROR_testerrortimer(&s_error_initres, ABBRUCH) ;
   err = initumgebung_objectcachemt(&svc->objectcache) ;
   if (err) goto ABBRUCH ;
   ++svc->resource_count ;

   ONERROR_testerrortimer(&s_error_initres, ABBRUCH) ;
   err = initumgebung_logwritermt(&svc->ilog) ;
   if (err) goto ABBRUCH ;
   ++svc->resource_count ;
// TEXTDB:END
   ...
   return 0 ;
ABBRUCH:
   (void) freemultithread_umgebungservices(svc) ;
   LOG_ABORT(err) ;
   return err ;
}
...
#define TEST(CONDITION) TEST_ONERROR_GOTO(CONDITION,unittest_umgebung_services_multithread,ABBRUCH)
...
static int test_initfree(void)
{
   ...
   TEST(0 == initmultithread_umgebungservices(&svc)) ;
   ...
   TEST(0 == freemultithread_umgebungservices(&svc)) ;
   TEST(0 == svc.resource_count) ;
   TEST(svc.ilog.object    == &g_main_logwriter) ;
   TEST(svc.ilog.functable == (log_it*)&g_main_logwriter_interface) ;
   TEST(0 == svc.objectcache.object) ;
   ...
   for(int i = 0; i < 3; ++i) {
      ...
      TEST(EINVAL+i == initmultithread_umgebungservices(&svc)) ;
      TEST(0 == svc.resource_count) ;
      TEST(svc.ilog.object    == &g_main_logwriter) ;
      TEST(svc.ilog.functable == (log_it*)&g_main_logwriter_interface) ;
      TEST(0 == svc.objectcache.object) ;
...
int unittest_umgebung_services_multithread()
{
