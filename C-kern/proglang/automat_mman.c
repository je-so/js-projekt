...
/* define: memory_page_PAGESIZE
 * Die Größe einer <memory_page_t> kodiert als <pagesize_e>. */
#define memory_page_PAGESIZE pagesize_262144
...
static int new_memorypage(/*out*/memory_page_t ** page)
{
   ...
   if (! PROCESS_testerrortimer(&s_automat_mman_errtimer, &err)) {
      err = ALLOC_PAGECACHE(memory_page_PAGESIZE, &memblock);
   }

   if (err) goto ONERR;

   // set out
   *page = (memory_page_t*) memblock.addr;
...
{
   int err;
   memblock_t memblock = memblock_INIT(memory_page_SIZE, (uint8_t*) page);
...
static int test_memorypage(void)
{
   ...
   // TEST memory_page_PAGESIZE
   TEST(memory_page_SIZE == pagesizeinbytes_pagecache(memory_page_PAGESIZE));
