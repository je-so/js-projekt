...
int dnsname_ipaddr(const ipaddr_t* addr, cstring_t* dns_name)
{
   ...
   for (;;) {
      err = getnameinfo( addr->addr, addr->addrlen, str_cstring(dns_name), (socklen_t) capacity_cstring(dns_name), 0, 0, NI_IDN|NI_IDN_ALLOW_UNASSIGNED|NI_NAMEREQD) ;
      if (err) {
         if (  EAI_OVERFLOW == err
...
int dnsnameace_ipaddr(const ipaddr_t * addr, cstring_t * dns_name)
{
   ...
   for (;;) {
      err = getnameinfo(addr->addr, addr->addrlen, str_cstring(dns_name), (socklen_t) capacity_cstring(dns_name), 0, 0, NI_NAMEREQD);
      if (err) {
         if (  EAI_OVERFLOW == err
               && capacity_cstring(dns_name) < 4096) {
            err = allocate_cstring(dns_name, 2*capacity_cstring(dns_name));
...
int numericname_ipaddr(const ipaddr_t * addr, cstring_t * numeric_name)
{
   ...
   for (;;) {
      err = getnameinfo( addr->addr, addr->addrlen, str_cstring(numeric_name), (socklen_t) capacity_cstring(numeric_name), 0, 0, NI_NUMERICHOST) ;
      if (err) {
         if (  EAI_OVERFLOW == err
