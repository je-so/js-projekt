...
static int read_buffer(int fd, const size_t buffer_maxsize, uint8_t buffer[buffer_maxsize], size_t buffer_offset, /*out*/size_t * buffer_size, /*out*/size_t * line_end)
{
   ...
   for (;;) {
      ...
      if (!read_size) {
         if (buffer_offset) {
            char insteadof[2] = { (char) buffer[buffer_offset-1], 0 };
            err = EINVAL;
            TRACE_ERRLOG(log_flags_NONE, FILE_HEADER_NAME, "", PROC_SELF_MAPS);
   ...
   for (size_t buffer_offset = 0;;) {
      ...
      do {
         ...
         if (scanned_items != 10) {
            err = EINVAL;
            TRACE_ERRLOG(log_flags_NONE, FILE_HEADER_NAME, "", PROC_SELF_MAPS);
            TRACE_NOARG_ERRLOG(log_flags_NONE, PARSEERROR_WRONG_FORMAT);
            goto ONERR;
         }
