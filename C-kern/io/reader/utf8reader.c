...
int init_utf8reader(/*out*/utf8reader_t * utfread, const char * filepath, const struct directory_t * relative_to/*0 => current working dir*/)
{
   ...
   // TODO: implement init_utf8reader
   err = ENOSYS ;
   if (err) goto ONABORT ;
...
static int test_initfree(directory_t * tempdir)
{
   ...
   TEST(ENOSYS == init_utf8reader(&utfread, "grow", tempdir)) ;
   TEST(0 == removefile_directory(tempdir, "grow")) ;

   // TEST initsinglebuffer_utf8reader, free_utf8reader
...
static int test_read(directory_t * tempdir)
{
   ...
   TEST(0 == initsinglebuffer_utf8reader(&utfread, "text", tempdir)) ;
   ...
   TEST(0 == initsinglebuffer_utf8reader(&utfread, "illseq", tempdir)) ;
   old = utfread ;
...
static int test_skipline(directory_t * tempdir)
{
   ...
   for (unsigned i = 0; i < sizeof(buffer); ++i) {
      ...
      TEST(0 == initsinglebuffer_utf8reader(&utfread, "newline", tempdir)) ;
      utfread.pos.line   = i ;
...
static int test_match(directory_t * tempdir)
{
   ...
   for (unsigned i = 0; i < sizeof(buffer); ++i) {
      matchedsize = 512 ;
      TEST(0 == initsinglebuffer_utf8reader(&utfread, "match", tempdir)) ;
