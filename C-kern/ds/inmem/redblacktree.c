...
static int test_initfree(void)
{
   ...
   typeadapt_member_t   nodeadapt = typeadapt_member_INIT(cast_typeadapt(&typeadapt, testadapt_t, testnode_t, uintptr_t), offsetof(testnode_t, node)) ;
   redblacktree_t       tree      = redblacktree_FREE ;
   ...
   // TEST redblacktree_FREE
   typeadapt_member_t emptyadapt = typeadapt_member_FREE ;
   TEST(0 == tree.root) ;
   ...
   typeadapt_member_t empty_nodeadapt = typeadapt_member_FREE ;
   tree = (redblacktree_t)redblacktree_FREE ;
...
static int test_insertremove(void)
{
   ...
   for (int i = (int)lengthof(*nodes)-1; i >= 0; --i) {
      TEST(0 == insert_redblacktree(&tree, &(*nodes)[i].node)) ;
      if (!(i%100)) TEST(0 == invariant_redblacktree(&tree)) ;
   }
   TEST(0 == invariant_redblacktree(&tree)) ;
...
static int test_iterator(void)
{
   ...
   typeadapt_member_t   nodeadapt = typeadapt_member_INIT(cast_typeadapt(&typeadapt, testadapt_t, testnode_t, uintptr_t), offsetof(testnode_t, node)) ;
   redblacktree_t       tree      = redblacktree_INIT(0, nodeadapt) ;
   redblacktree_t       emptytree = redblacktree_FREE ;
   redblacktree_iterator_t iter   = redblacktree_iterator_FREE ;
   ...
   init_redblacktree(&tree, &nodeadapt) ;
   for (unsigned i = 0; i < lengthof(nodes); ++i) {
      testnode_t * node = &nodes[(13*i) % lengthof(nodes)] ;
      TEST(0 == insert_redblacktree(&tree, &node->node)) ;
   }
   TEST(0 == invariant_redblacktree(&tree)) ;

   // TEST redblacktree_iterator_FREE
   TEST(0 == iter.next) ;
   ...
   TEST(&nodes[lengthof(nodes)-1].node == iter.next) ;
