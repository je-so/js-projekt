 ...
 * Add new element of type TYPENAME to end of queue (last).
 * The address of allocated element is returned in newelem.
 * It is in an unitialized state.
 * In case of no memory ENOMEM is returned and nothing is done. */
int insert_syncqueue(syncqueue_t * syncqueue, TYPENAME ** newelem);

/* function: insert2_syncqueue
 * Add new element of type TYPENAME to end of queue (last).
 * The address of allocated element is returned in newelem.
 * It is in an unitialized state. The extra parameter elemsize
 * allows to add extrasize at the end of the element.
 * In case of no memory ENOMEM is returned and nothing is done.
 *
 * Precondition:
 * o elemsize >= sizeof(**newelem) */
int insert2_syncqueue(syncqueue_t * syncqueue, uint8_t elemsize, TYPENAME ** newelem);

/* function: remove_syncqueue
 * Removes elem of type TYPENAME from the queue by overwriting it with the last.
...
int remove_syncqueue(syncqueue_t * syncqueue, TYPENAME * elem, void (*initmove_elem)(TYPENAME * dest, TYPENAME * src));
...
void addtofreelist_syncqueue(syncqueue_t * syncqueue, struct dlist_t * freelist, TYPENAME * elem);
...
int compact_syncqueue(syncqueue_t * syncqueue, TYPENAME elem_t, struct dlist_t * freelist, void (*initmove_elem)(TYPENAME * dest, TYPENAME * src));

/* function: compact2_syncqueue
 * Called from macro <compact_syncqueue>.
 * Do not call this function directly. Use macro <compact_syncqueue> to ensure type safety. */
