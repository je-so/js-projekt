/* title: Ad-hoc-Memory-Manager
   ...
   Implements a simple memory heap for type <automat_t>.
   Every heap manages all states of one or more automaton.
   But a single automaton can not share states between different heaps.

   TODO: replace with a general memory manager which allows to manage memory groups.
         Memory groups are used to simplify freeing and allocating of memory blocks.
